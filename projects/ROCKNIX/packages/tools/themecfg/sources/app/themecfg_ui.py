...
class App(object):
    def __init__(self):
        ...
        # backgrounds by {system:theme}.jpg, so each entry MUST be the system's
        # theme key: the favorites auto-collection themes as "auto-favorites"
        # (NOT "favorites"; see ES CollectionSystemManager), tools as "tools".
        self.bgcats = self.systems + [c for c in ("auto-favorites", "tools")
    ...
    def draw_bg(self):
        ...
        # show a friendly label but keep the theme-key category for the filename
        label = {"auto-favorites": "favorites"}.get(cat, cat)
        pyxel.text(6, 16, "BACKGROUND:  < %s >" % label.upper(), HEAD)
        if not self.bg_files:
