...
# --- the Steam/Decky plugin's shared library (DUCKTALE-CONTROLS, our aanze fork
# of thefiqs/rocknix-control) -------------------------------------------------
# On ROCKNIX, Decky installs under /storage/homebrew and per-plugin settings
# live in /storage/homebrew/settings/<plugin-folder>/ (Decky keys the dir off
# the plugin's folder name, not its display name). This is the file we two-way
# sync with so a profile shows up in both the ES tool and the Steam plugin.
PLUGIN_DIR = "/storage/homebrew/settings/ducktale-controls"
