...
# --------------------------------------------------------------------------- #
# live clock snapshot (used to detect which profile is currently applied)
# --------------------------------------------------------------------------- #
def live_clocks():
    """Current CPU/GPU caps as a profile-shaped dict (cpu_policyN_max/min, gpu_*)."""
    out = {}
    for pol in list_cpu_policies():
        n = pol["num"]
        if pol["scaling_max"] is not None:
            out["cpu_policy%d_max" % n] = pol["scaling_max"]
        if pol["scaling_min"] is not None:
            out["cpu_policy%d_min" % n] = pol["scaling_min"]
    gpu = find_gpu()
    if gpu:
        if gpu["max"] is not None:
            out["gpu_max"] = gpu["max"]
        if gpu["min"] is not None:
            out["gpu_min"] = gpu["min"]
    return out
